...
DDPClient.prototype._message = function(data) {
  ...
  // minimal heartbeat response for ddp pre2
    ...
    //log(TAG, "RECEIVED PING FROM SERVER!!!!!!");
    self._send(
...
DDPClient.prototype.connect = function(connected) {
  ...
  if(pingSender && ENABLE_PING) {
    pingOutstanding = false
    clearInterval(pingSender)
    //log(TAG, "Cleared pinger for new connection..");
  }
  ...
  if (connected) {
    ...
    if(connectListener) {
      ...
      //log(TAG, 'Removed previous "connected" listener.');
    }

    if(failedListener) {
      self.removeListener("failed", failedListener)
      failedListener = undefined
      //log(TAG, 'Removed previous "failed" listener.');
    }

    //log(TAG, Adding new listeners..');
    self.addListener("connected", connectListener = function() {
      log(TAG, 'CONNECTED EVENT');
      self._clearReconnectTimeout();

      // log(TAG, 'SENDING PING');
      // self._send({
      //   msg    : "ping"
      // });
      //log(TAG, 'Setting up ping transmitter..');
      if(ENABLE_PING) {
        pingSender = setInterval(() => {
          if(!pingOutstanding) {
            ...
            self._send({msg:"ping"})
              ...
              log(TAG, 'Outstanding ping was not resolved, closing connection..');
              self.close()
          }
        }, PING_TIMEOUT)
      }
...
DDPClient.prototype.close = function() {
  log('[' + new Date().toISOString() + ']', TAG, 'CALLED CLOSE FUNCTION');
  var self = this;
  self._isClosing = true;
  self.socket.close()
  self.socket._stream.destroy()
  // log(TAG, 'Removing all "connected" and "failed" listeners..');
