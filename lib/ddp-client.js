...
DDPClient.prototype._prepareHandlers = function() {
  ...
  self.socket.on("close", socketCloseListener = function(event) {
    if(pingSender) {
      console.log(TAG, 'Clearing ping transmitter (from closed event)..');
    ...
    if(socketCloseListener) {
      console.log(TAG, 'Removing socket close listener..');
...
DDPClient.prototype._message = function(data) {
  ...
  // minimal heartbeat response for ddp pre2
    ...
    console.log(TAG, "RECEIVED PING FROM SERVER!!!!!!");
    self._send(
...
DDPClient.prototype.connect = function(connected) {
  ...
  if(pingSender) {
    pingOutstanding = false
    clearInterval(pingSender)
    console.log(TAG, "Cleared pinger for new connection..");
  }
  ...
  if (connected) {
    ...
    if(connectListener) {
      ...
      console.log(TAG, 'Removed previous "connected" listener.');
    }

    if(failedListener) {
      self.removeListener("failed", failedListener)
      failedListener = undefined
      console.log(TAG, 'Removed previous "failed" listener.');
    }
    ...
    self.addListener("connected", connectListener = function() {
      ...
      // console.log(TAG, 'SENDING PING');
      // self._send({
      //   msg    : "ping"
      // });
      console.log(TAG, 'Setting up ping transmitter..');
      pingSender = setInterval(() => {
        if(!pingOutstanding) {
          ...
          self._send({msg:"ping"})
            ...
            console.log(TAG, 'Outstanding ping was not resolved, closing connection..');
            self.close()
        }
      }, 5000)
...
DDPClient.prototype.close = function() {
  console.log(TAG, 'CALLED CLOSE FUNCTION');
  var self = this;
