...
DDPClient.prototype.connect = function(connected) {
  ...
  if (connected) {
    if(connectedFunc) { //Func is same as previous
      if(connectListener) { //Remove existing connected listener
        self.removeListener("connected", connectListener)
      }
      if(failedListener) { //Remove existing failed listener
...
DDPClient.prototype.close = function() {
  ...
  // if(pingSender) {
  //   console.log(TAG, 'Clearing ping transmitter (from close function)..');
  //   clearInterval(pingSender)
  // }
};
