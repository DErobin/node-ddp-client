console.log('[' + new Date() + ']', TAG,"use strict";
...
const TAG = '[LIB]:'
const V = 'test2'
console.log('[' + new Date() + ']', TAG, 'VERSION:', V);
...
DDPClient.prototype._prepareHandlers = function() {
  ...
  self.socket.on("close", socketCloseListener = function(event) {
    if(pingSender) {
      console.log('[' + new Date() + ']', TAG, 'Clearing ping transmitter (from closed event)..');
    ...
    if(socketCloseListener) {
      console.log('[' + new Date() + ']', TAG, 'Removing socket close listener..');
...
DDPClient.prototype._message = function(data) {
  ...
  // minimal heartbeat response for ddp pre2
    ...
    console.log('[' + new Date() + ']', TAG, "RECEIVED PING FROM SERVER!!!!!!");
    self._send(
    ...
    console.log('[' + new Date() + ']', TAG, "RECEIVED PONG!!!!!!");
  }
};
...
DDPClient.prototype.connect = function(connected) {
  ...
  if(pingSender) {
    pingOutstanding = false
    clearInterval(pingSender)
    console.log('[' + new Date() + ']', TAG, "Cleared pinger for new connection..");
  }
  ...
  if (connected) {
    ...
    if(connectListener) {
      ...
      console.log('[' + new Date() + ']', TAG, 'Removed previous "connected" listener.');
    }

    if(failedListener) {
      self.removeListener("failed", failedListener)
      failedListener = undefined
      console.log('[' + new Date() + ']', TAG, 'Removed previous "failed" listener.');
    }

    console.log('Adding new listeners..');
    self.addListener("connected", connectListener = function() {
      console.log('[' + new Date() + ']', TAG, 'CONNECTED EVENT');
      self._clearReconnectTimeout();

      // console.log('[' + new Date() + ']', TAG, 'SENDING PING');
      // self._send({
      //   msg    : "ping"
      // });
      console.log('[' + new Date() + ']', TAG, 'Setting up ping transmitter..');
      pingSender = setInterval(() => {
        if(!pingOutstanding) {
          ...
          self._send({msg:"ping"})
            ...
            console.log('[' + new Date() + ']', TAG, 'Outstanding ping was not resolved, closing connection..');
            self.close()
        }
      }, 10000)
    ...
    self.addListener("failed", failedListener = function(error) {
      console.log('[' + new Date() + ']', TAG, 'FAILED EVENT');
      self._isConnecting = false;
...
DDPClient.prototype.close = function() {
  console.log('[' + new Date() + ']', TAG, 'CALLED CLOSE FUNCTION');
  var self = this;
