...
const TAG = '[LIB]:'
const V = 'test2'
console.log('[' + new Date().toISOString() + ']', TAG, 'VERSION:', V);
...
DDPClient.prototype._prepareHandlers = function() {
  ...
  self.socket.on("close", socketCloseListener = function(event) {
    if(pingSender) {
      console.log('[' + new Date().toISOString() + ']', TAG, 'Clearing ping transmitter (from closed event)..');
    ...
    if(socketCloseListener) {
      console.log('[' + new Date().toISOString() + ']', TAG, 'Removing socket close listener..');
      self.socket.removeListener("close", socketCloseListener)
    }
...
DDPClient.prototype._message = function(data) {
  ...
  // minimal heartbeat response for ddp pre2
    ...
    console.log('[' + new Date().toISOString() + ']', TAG, "RECEIVED PING FROM SERVER!!!!!!");
    self._send(
    ...
    console.log('[' + new Date().toISOString() + ']', TAG, "RECEIVED PONG!!!!!!");
  }
};
...
DDPClient.prototype.connect = function(connected) {
  ...
  if(pingSender) {
    pingOutstanding = false
    clearInterval(pingSender)
    console.log('[' + new Date().toISOString() + ']', TAG, "Cleared pinger for new connection..");
  }
  ...
  if (connected) {
    ...
    if(connectListener) {
      self.removeListener("connected", connectListener)
      connectListener = undefined
      console.log('[' + new Date().toISOString() + ']', TAG, 'Removed previous "connected" listener.');
    }

    if(failedListener) {
      self.removeListener("failed", failedListener)
      failedListener = undefined
      console.log('[' + new Date().toISOString() + ']', TAG, 'Removed previous "failed" listener.');
    }

    console.log('Adding new listeners..');
    self.addListener("connected", connectListener = function() {
      console.log('[' + new Date().toISOString() + ']', TAG, 'CONNECTED EVENT');
      self._clearReconnectTimeout();

      // console.log('[' + new Date().toISOString() + ']', TAG, 'SENDING PING');
      // self._send({
      //   msg    : "ping"
      // });
      console.log('[' + new Date().toISOString() + ']', TAG, 'Setting up ping transmitter..');
      pingSender = setInterval(() => {
        if(!pingOutstanding) {
          ...
          self._send({msg:"ping"})
            ...
            console.log('[' + new Date().toISOString() + ']', TAG, 'Outstanding ping was not resolved, closing connection..');
            self.close()
        }
      }, 10000)
    ...
    self.addListener("failed", failedListener = function(error) {
      console.log('[' + new Date().toISOString() + ']', TAG, 'FAILED EVENT');
      self._isConnecting = false;
...
DDPClient.prototype.close = function() {
  console.log('[' + new Date().toISOString() + ']', TAG, 'CALLED CLOSE FUNCTION');
  var self = this;
  self._isClosing = true;
  self.socket.close()
  self.socket._stream.destroy()
  console.log('Removing all "connected" and "failed" listeners..');
